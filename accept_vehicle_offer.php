...
if(isset($_GET['offer'])){
    $offer = getVehicleOffer($_GET['offer']);
    if($offer != null){
        if($_SESSION['logged']){
            if($_SESSION['id'] == getVehicle($offer->getFor_id())->getAuthor()){
    ?>    
    <div class="title">Akceptacja oferty:</div>
    <div class="info">SpeedMTrans pobiera  10% prowizji od akceptacji oferty. Po  uiszczeniu opłaty otrzymasz dane kontaktowe do klienta na swój adres email. Dalszy przebieg transakcji nie jest kontrolowany przez nasz serwis nie ponosimy odpowiedzialności za dalsze rozliczenia między Tobą i klientem.</div>
    <br>
    ...
    <form id="p24_form" action="php/payment.php" method="post">
        <input name="type" value="vehicle" hidden/>
